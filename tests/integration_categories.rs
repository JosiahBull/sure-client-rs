...
async fn test_category_crud_lifecycle() {
    ...
    let update_request = UpdateCategoryRequest {
        category: UpdateCategoryData {
            name: Some(format!("Updated Category {}", timestamp)),
            color: Some("#3366FF".to_string()),
            lucide_icon: Some("tag".to_string()),
            ..Default::default()
        },
    };

    let updated = client
        .update_category(&created.id, &update_request)
        .await
    ...
    let categories = client
        .get_categories().page(1).per_page(100).call()
        .await
...
async fn test_list_categories_by_classification() {
    ...
    let expense_categories = client
        .get_categories().page(1).per_page(50).classification(Classification::Expense).call()
        .await
    ...
    let income_categories = client
        .get_categories().page(1).per_page(50).classification(Classification::Income).call()
        .await
...
async fn test_create_income_category() {
    ...
    let create_request = CreateCategoryRequest {
        category: CreateCategoryData {
            ...
            lucide_icon: Some("dollar-sign".to_string()),
            parent_id: None,
        },
    };
...
async fn test_category_with_parent() {
    ...
    let parent_request = CreateCategoryRequest {
        category: CreateCategoryData {
            ...
            lucide_icon: Some("folder".to_string()),
            parent_id: None,
        },
    };

    let parent = client
        .create_category(&parent_request)
        .await
    ...
    let child_request = CreateCategoryRequest {
        category: CreateCategoryData {
            ...
            lucide_icon: Some("file".to_string()),
            parent_id: Some(parent.id.clone()),
        },
    };

    let child = client
        .create_category(&child_request)
        .await
...
async fn test_list_categories_pagination() {
    ...
    // Test first page
    let page1 = client
        .get_categories().page(1).per_page(10).call()
        .await
    ...
    if page1.items.categories.len() == 10 {
        let page2 = client
            .get_categories().page(2).per_page(10).call()
            .await
...
async fn test_update_category_classification() {
    ...
    let create_request = CreateCategoryRequest {
        category: CreateCategoryData {
            ...
            lucide_icon: Some("credit-card".to_string()),
            parent_id: None,
        },
    };
    ...
    let update_request = UpdateCategoryRequest {
        category: UpdateCategoryData {
            classification: Some(Classification::Income),
            ..Default::default()
        },
    };
